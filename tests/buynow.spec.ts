...
test('test', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Email Id or mobile number' }).fill('shedgeadi31@gmail.com');
  await page.getByRole('textbox', { name: 'Password Show password' }).click();
  await page.getByRole('textbox', { name: 'Password Show password' }).fill('Adi@#3112');
